...
from werkzeug.utils import secure_filename # 파일이름, 경로데 대한 기본적인 보안

# flask server 보안 규칙
# 1. html 문서들은 render_template으로 로드시
# 반드시 templates 폴더 내에 존재해야한다
# 2. 모든 경로에 대해 접근 불가
# 단, static 경로만 접근 가능
...
def index():
    ...
    return '''
        ...
        '''
@app.route('/detect', methods=['GET','POST']) # root 경로에서 넘어 온 이미지를 받아오는 웹페이지
def detect():
    # request 관련 페이지들은
    # route 설정 시 반드시 전송받식을 정의해야 한다

    # GET -> request.args['Key값']
    # POST -> request.form['Key값]
    # file -> request.jiles['key값']
...
# 내가 직접 실행(run)시 내장 변수 __name__이 __main__ 변한다
